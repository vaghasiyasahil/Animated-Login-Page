...
class _ManLoginState extends State<ManLogin> {
  ...
  void initState() {
    ...
    rootBundle.load(animationURL).then((data) {
      ...
      setState(() => _manArtboard = artboard);
    });
  }

  void handleFocus() {
    isCheking?.change(true);
    isHandUp?.change(false); // Reset glasses on state when focusing
  }

  void handlePasswordFocus() {
    isCheking?.change(false);
    isHandUp?.change(true);
    numLook?.change(0); // Reset eye tracking when focusing on password
  }
